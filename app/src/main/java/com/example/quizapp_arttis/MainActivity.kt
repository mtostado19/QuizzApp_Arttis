...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (topicQuestion in Temas){
            ...
            var questionAnswers = questionsTxt.map { it.split("*") } // separa las preuntas de las respuestas

            var questionResult =  mutableListOf<List<List<String>>>() // crea listas para guardar el conjunto de lista de pregunta y listas de respuestan en el mismo conjunto
            ...
            if (topicQuestion==QuestionGeografia){
                if (banderaOtaku){
                    for (qts in 0..tostazzz-1+numeroFaltantes){
                        ...
                        QuestionList.add(Question(questionResult[qts][0][0],questionResult[qts][1][0],limitAnswerd.shuffled(),"not","Geografia",false))
                    }
                    banderaOtaku = false
                }
                else{
            ...
            if (topicQuestion==QuestionVideojuegos){
                if (banderaOtaku){
                    for (qts in 0..tostazzz-1+numeroFaltantes){
                        var limitAnswerd = questionResult[qts][1].take(dificultad+1)
                        QuestionList.add(Question(questionResult[qts][0][0],questionResult[qts][1][0],limitAnswerd.shuffled(),"not","Videojuegos",false))
                    }
                    banderaOtaku = false
                }
                else{
            ...
            if (topicQuestion==QuestionHistoria){
                if (banderaOtaku){
                    for (qts in 0..tostazzz-1+numeroFaltantes){
                        var limitAnswerd = questionResult[qts][1].take(dificultad+1)
                        QuestionList.add(Question(questionResult[qts][0][0],questionResult[qts][1][0],limitAnswerd.shuffled(),"not","Historia",false))
                    }
                    banderaOtaku = false
                }
                else{
            ...
            if (topicQuestion==QuestionCiencia){
                if (banderaOtaku){
                    for (qts in 0..tostazzz-1+numeroFaltantes){
                        var limitAnswerd = questionResult[qts][1].take(dificultad+1)
                        QuestionList.add(Question(questionResult[qts][0][0],questionResult[qts][1][0],limitAnswerd.shuffled(),"not","Ciencia",false))
                    }
                    banderaOtaku = false
                }
                else{
            ...
            if (topicQuestion==QuestionPeliculas){
                if (banderaOtaku){
                    for (qts in 0..tostazzz-1+numeroFaltantes){
                        var limitAnswerd = questionResult[qts][1].take(dificultad+1)
                        QuestionList.add(Question(questionResult[qts][0][0],questionResult[qts][1][0],limitAnswerd.shuffled(),"not","Peliculas",false))
                    }
                    banderaOtaku = false
                }
                else{
            ...
            if (topicQuestion==QuestionProgramación){
                if (banderaOtaku){
                    for (qts in 0..tostazzz-1+numeroFaltantes){
                        var limitAnswerd = questionResult[qts][1].take(dificultad+1)
                        QuestionList.add(Question(questionResult[qts][0][0],questionResult[qts][1][0],limitAnswerd.shuffled(),"not","Programacion",false))
                    }
                    banderaOtaku = false
                }
                else{
